...
If there is no parameter (but by default = not), then create REG_DWORD. For disable value (HIGHLY NOT RECOMMENDED) [255](https://learn.microsoft.com/en-en/troubleshoot/windows-server/networking/configure-ipv6-in-windows)
...
Если параметра нет (а по умолчанию нет), то создать REG_DWORD. Чтобы отключить (КРАЙНЕ НЕ РЕКОМЕНДУЕТСЯ), значение [255](https://learn.microsoft.com/ru-ru/troubleshoot/windows-server/networking/configure-ipv6-in-windows)
